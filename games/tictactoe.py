    ...
    def attempt_move(self, col, row):
        if self.game_active and 0 <= col < self.dim and 0 <= row < self.dim:
            if self.board[row][col] == -1:
                ...
                # particles (same as your version)
        ...
        for [dx, dy] in [(0, 1), (1, 0), (1, 1), (-1, 1)]:
            for x in range(self.dim):
                for y in range(self.dim):
                    if (
                        0 <= (x + (dx * (self.win_len - 1))) < self.dim
                        and 0 <= (y + (dy * (self.win_len - 1))) < self.dim
                    ):
            ...
            if event.type == pg.QUIT:
                ...
                if event.key in [pg.K_RETURN, pg.K_SPACE]:
                    if not self.game_active:
                        ...
                        self.attempt_move(self.ptr[0], self.ptr[1])
                    ...
                    self.ptr[1] = max(0, self.ptr[1] - 1)

                elif event.key == pg.K_DOWN:
                    self.ptr[1] = min(self.dim - 1, self.ptr[1] + 1)

                elif event.key == pg.K_LEFT:
                    self.ptr[0] = max(0, self.ptr[0] - 1)

                elif event.key == pg.K_RIGHT:
                    self.ptr[0] = min(self.dim - 1, self.ptr[0] + 1)
    ...
    def render_board(self):
        for x in range(self.dim):
            for y in range(self.dim):
                ...
                rect = self.o_surf.get_rect(
                    center=(x_pos + self.box_len // 2, y_pos + self.box_len // 2)
                )

                if self.board[(y, x)] == 0:
                    self.screen.blit(self.o_surf, rect)
                elif self.board[(y, x)] == 1:
                    self.screen.blit(self.x_surf, rect)
